...
class MyNode(DTROS):
    def __init__(self,node_name):
        ...
        self.im1 = cv2.imread(str(self.path)+"/src/stop.png",1)
        ...
        self.kp2, self.des2 = self.orb.detectAndCompute(self.im1, None)
    ...
    def camera_callback(self, data):
        ...
        #crop_img =cv_image[int((height)/2+ self.descenter):int((height)/2)+int(self.descenter+rows_to_watch),180:500,:]
        crop_img =cv_image[int((height)/2+ self.descenter):int((height)/2)+int(self.descenter+rows_to_watch),165:505,:]
        ...
        #image_to_detect_stop = cv2.bitwise_and(cv_image, cv_image, mask =image_to_detect_stop)[15:int(height*1.4),100:550,:]
        self.image_to_detect_stop = cv2.bitwise_and(cv_image, cv_image, mask =image_to_detect_stop)[0:int(height*1.5),200:450,:]
        #self.image_to_detect_stop = cv2.cvtColor(image_to_detect_stop,cv2.COLOR_BGR2GRAY)
    ...
    def detect_stop_sign(self):
        if not self.image_to_detect_stop is None:
            ...
            kp1, des1 = self.orb.detectAndCompute(self.image_to_detect_stop,None)
